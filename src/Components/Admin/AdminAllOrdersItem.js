...
const AdminAllOrdersItem = ({OrdersData,admin}) => {
    const item = OrdersData.cartItems[0];
    return (
        <Col xs="12" className="cart-item-body my-2 d-flex px-2 h-auto p-sm-30px">
            <Link to={`/Full-ecommerce-react/admin/allorders/${OrdersData._id}`} className='cart-item-body my-2 px-1 d-flex text-decoration-none h-auto flex-column flex-md-row align-items-center'>
                <img width="160px" height="197px"  src={BaseURL.getUri() + "/products/" + (item && item.product && item.product.imageCover || '')} alt="" />
                <div className="w-100 mx-2">
                    ...
                    <Row className="justify-content-center mt-2">
                        <Col sm="12" className=" d-flex flex-row justify-content-start">
                            ...
                            <div className="d-inline pt-2 cat-rate me-2 mx-2">{item && item.product &&item.product.ratingsAverage}</div>
                        </Col>
                    </Row>
                    <Row>
                        <Col sm="12" className="mt-1">
                            <div className="cat-text d-inline">Brand: </div>
                            <div className="barnd-text d-inline mx-1">{item && item.product &&item.product.brand} </div>
                        </Col>
                    </Row>
                    <Row>
